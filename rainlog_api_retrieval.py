...
if __name__ == '__main__':
    ...
    readings_revisions = pd.DataFrame({})
    for day in range(0, round((end - start) / np.timedelta64(1, "D"))+1):
        curr_day = start + pd.DateOffset(day)
        curr_readings_revisions = get_readings_with_metadata(curr_day, curr_day, region)
        if readings_revisions.shape[0] < 0:
            readings_revisions = curr_readings_revisions
        else:
            readings_revisions = pd.concat([readings_revisions, curr_readings_revisions])
        print(curr_day,curr_readings_revisions.shape[0],curr_readings_revisions.shape[1])
    ...
    readings_revisions.to_csv(outfile, sep='\t', encoding='utf-8')
